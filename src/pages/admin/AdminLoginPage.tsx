...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      navigate(RouteBuilder.adminDashboard())
    } else {
  ...
  return (
    <>
      ...
      <div className="login-page">
        <div className="login-hero" style={{ backgroundImage: 'url(/admin-hero.png)' }}>
          <div className="login-hero-content">
            <h1>Manage TGPL,<br />Keep projects on track,<br />one task at a time!</h1>
            {/* No subtext for admin, per signup page pattern */}
        ...
        <div className="login-form-panel">
          ...
          <div role="alert" aria-live="polite" style={{ width: '100%', maxWidth: '440px', marginBottom: (error || formErrors.submit) ? '1rem' : 0 }}>
            {error && <Alert type="error" title="Login failed">{error}</Alert>}
            {formErrors.submit && <Alert type="error" title="Login failed">{formErrors.submit}</Alert>}
          </div>

          <div className="login-card">
            <div className="login-title">
              <h2>Welcome back</h2>
              <p>Log in to continue managing TGPL.</p>
            </div>
            ...
            <div className="login-footer">
              <p>New here? <Link to={ROUTES.ADMIN_SIGNUP}>Create an account</Link></p>
            </div>
          </div>
