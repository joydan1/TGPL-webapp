...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      // Read from store directly — login() has already called store.login()
      // so the user is guaranteed to be set at this point
      ...
      // 403 + code='email_not_verified' → show resend banner
      if (result.statusCode === 403 && result.code === 'email_not_verified') {
  ...
  // clearError is stable (wrapped in useCallback with empty deps in useAuth)
  useEffect(() => {
